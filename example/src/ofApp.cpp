...
void ofApp::draw()
{
    ...
    for (int d = 0; d < kinects.size(); d++){
        ...
        if (texDepth[d].isAllocated())
            texDepth[d].draw(200, shiftY);
        
        if (texRGB[d].isAllocated())
            texRGB[d].draw(210 + texDepth[d].getWidth(), shiftY, dwHD, dhHD);
        
        if (texIR[d].isAllocated())
            texIR[d].draw(210 + texIR[d].getWidth(), texDepth[d].getHeight(), texIR[d].getWidth(), texIR[d].getHeight());
    }
